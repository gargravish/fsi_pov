# UBS Helix — Agentic Wealth & Banking Intelligence Platform (Google Cloud POV)

> **Purpose.** A single, self-contained, vibe-code-ready blueprint for a C‑level workshop demo that shows how the **Google Cloud Agentic Data Platform**, with **BigQuery as the intelligence core**, solves UBS's most valuable business problems — built natively on **BigQuery AI** (autonomous embeddings, AI.FORECAST/TimesFM, TabularFM, BigQuery Graph/GQL, Conversational Analytics, BigFrames) and a layer of **collaborating agents** (a Data Engineering agent and a Data Scientist agent talking over **A2A**, plus a Conversational Analytics agent), orchestrated with **ADK**.
>
> **Audience.** UBS Group/divisional CxOs (CEO/CIO/COO/CDO of GWM, P&C, Asset Management) and their architects — and the engineer (or AI pair‑programmer) who will build the demo from this doc.
>
> **Status.** Design blueprint — POV / demo. Not production. **All data is synthetic.** No real UBS or Credit Suisse data is used.
>
...
We will build **"UBS Helix"** — an agentic wealth-and-banking intelligence web app on the Google Cloud Agentic Data Platform. A unified BigQuery lakehouse ingests *deliberately fragmented* synthetic data from **two legacy estates — "UBS" and "Credit Suisse"** (the real-world integration completing end‑2026) — and a set of **collaborating agents** sit on top to deliver the outcomes UBS cares about most:
...
5. **Conversational analytics — "Ask UBS"** — any banker/exec queries the whole estate in English. **Conversational Analytics API (Gemini Data Analytics agent)**.
...
## 1. Customer Context — UBS Group (Research Findings)

This grounds the synthetic data and the narrative so the demo feels authentic to a UBS audience.
...
### 1.2 The defining event — the Credit Suisse integration
- UBS acquired **Credit Suisse** (2023) and is on track to **substantially complete the integration by end‑2026**, with the remaining work centred on **client‑account migrations** and **decommissioning duplicate infrastructure**.
...
> **Why this is the perfect POV spine.** Two giant banks, two of everything — two client masters, two product catalogues, two booking systems, two document stores, overlapping clients who bank with *both*. The integration is fundamentally a **data‑unification + AI‑activation** problem. UBS Helix tells exactly that story: *fragmentation → one governed BigQuery truth → agents that drive wealth growth, retention, forecasting, compliance and advisor productivity.*

### 1.3 Their current AI posture (and our opening)
- UBS has invested heavily in GenAI on **Microsoft Azure / Azure OpenAI**: an internal **AI Hub**, the **"UBS Red"** assistant deployed to ~**30,000** employees, and ~**60,000** investment‑advice/product documents digitised into a queryable knowledge base for Client Advisors.
...
> *"You're merging the two largest pools of private wealth on earth into one bank. Your data lives in two of everything. UBS Helix turns that into your biggest asset: a single, governed, AI‑native lakehouse in BigQuery where collaborating agents drive net‑new‑money, retention, cross‑sell, forecasting and financial‑crime defence — without armies of pipelines or a separate ML platform to run."*

---

## 2. Business Objectives → Agentic Use Cases (Mapping)

| # | UBS business objective | Agentic use case (demo) | Primary GCP / BigQuery AI capability |
|---|---|---|---|
| 1 | **Finish the CS integration** | "Unify & Resolve" builds one Client 360 from two banks | **AI.GENERATE_TABLE** mapping + **autonomous embeddings** + **VECTOR_SEARCH** + **AI.GENERATE_BOOL** entity resolution → run by the **Data Engineering agent** |
...
| 5 | **Democratise analytics** | "Ask UBS" — chat with the whole estate in plain English | **Conversational Analytics API (Gemini Data Analytics agent)** |
...
```
                    ...
                    │   ├─ Data Engineering agent ◄──┼─► │  agent)  "Ask UBS"        │
            ...
            ┌───────────────────────┼───────────────────────────────────────────────┐
            │                       │   BigQuery (the brain) — dataset UBS_POV        │
            │  ┌─────────────┐  ┌───┴─────────────┐  ┌──────────────────┐  ┌────────┐ │
            │  │ RAW zone    │  │ CURATED (canon.)│  │ AI / ML layer    │  │ GRAPH  │ │
            │  │ UBS + CS    │→ │ clients,accounts│→ │ autonomous embeds│  │ client │ │
            ...
            └─────────────────────────────┼────────────────────┼──────────────────────┘
                ...
                └──────────────────┘                 └──────────────────────────────┘
            Cloud Storage: gs://ubs_pov  (raw fragmented files: UBS/* and CreditSuisse/*)
```
...
| Feature | What it does | Where used in UBS Helix |
|---|---|---|
...
| **AI.GENERATE_TABLE** | Structured (typed columns) output from a prompt | Normalising messy UBS/CS records → canonical client/account fields |
...
| **Conversational Analytics API** | NL data agent (text/tables/charts + SQL) over BQ | "Ask UBS" |
...
The credibility of the demo depends on data that *looks like a post‑acquisition integration* and then gets unified. We generate two legacy estates — **UBS** and **Credit Suisse** — in **different formats with intentionally inconsistent schemas**, land them in GCS, and let the platform conform them.

> Hosted on: `gs://ubs_pov/raw/UBS/...` and `gs://ubs_pov/raw/CreditSuisse/...`; loaded into BigQuery dataset **`UBS_POV`** (`us-central1`).
...
4. **Cross‑bank overlap (the integration signal).** ~**20–25%** of clients are **dual‑banked** — they exist in *both* UBS and CS with mutated attributes. The platform must *rediscover* this (entity resolution) — we never write the true id into the source files.
...
|---|---|---|---|
| **UBS** | Clients | CSV | `client_id`, CH dates `DD.MM.YYYY`, CHF, German field names (`nachname`, `wohnort`) |
| **UBS** | Portfolios / mandates | Fixed‑width (mainframe) | column positions, `YYYYMMDD`, mandate codes not labels |
| **UBS** | Positions / holdings | Parquet | ISIN, asset‑class codes, CHF valuation |
| **UBS** | Advisors (CA/RM) | Excel `.xlsx` | desk, booking centre, AuM book, language skills |
| **Credit Suisse** | Clients | JSON (nested) | nested `client{}`/`address{}`, ISO dates, `cifNumber`, mixed CHF/USD |
| **Credit Suisse** | Accounts / mandates | XML | `<Account>` elements, product code lists, EUR/USD |
| **Credit Suisse** | Transactions | NDJSON event stream | per‑txn events, value date as epoch ms, FX legs |
...
synthetic_data/
  ...
  banks/
    ubs_clients_csv.py
    ubs_portfolios_fixedwidth.py
    ubs_positions_parquet.py
    ubs_advisors_xlsx.py
    cs_clients_json.py
    cs_accounts_xml.py
...
- Result: `raw_ubs_*` and `raw_cs_*` tables with deliberately different column names/types.
...
```sql
-- Normalise Credit Suisse raw clients into canonical shape
CREATE OR REPLACE TABLE UBS_POV.stg_client_cs AS
SELECT
  AI.GENERATE_TABLE(
  ...
  'credit_suisse' AS source_bank,
  t.cifNumber AS source_key
FROM UBS_POV.raw_cs_clients t;
```
Repeat per source → `stg_client_ubs`, `stg_client_cs`, then `UNION ALL` into `UBS_POV.client_inbound`.
...
2. Adjudicate candidate pairs with **AI.GENERATE_BOOL** ("Are these the same person/entity across UBS and Credit Suisse?").
...
### 5.3 Output
Populates the CURATED tables (Section 6). The **"Unify & Resolve"** panel visualizes: sources discovered, % fields mapped, dual‑banked clients found, resolution accuracy, and a before/after record example (messy CS JSON → clean canonical client).

---

## 6. Canonical BigQuery Data Model (CURATED) — dataset `UBS_POV`

Single consolidated dataset `UBS_POV` (logical zones via table prefixes), portable to a four‑dataset layout if preferred. Core curated tables (PK in **bold**):
...
**Property graph:**
```sql
CREATE PROPERTY GRAPH UBS_POV.client_graph
NODE TABLES (
  UBS_POV.clients   KEY(client_id)   LABEL Client   PROPERTIES(client_id, full_name, segment_tier, booking_centre, household_id),
  UBS_POV.households KEY(household_id) LABEL Household PROPERTIES(household_id, member_count, total_aum_usd),
  UBS_POV.products  KEY(product_id)  LABEL Product  PROPERTIES(product_id, product_type, name),
  UBS_POV.accounts  KEY(account_id)  LABEL Account  PROPERTIES(account_id, account_type, booking_centre),
  UBS_POV.advisors  KEY(advisor_id)  LABEL Advisor  PROPERTIES(advisor_id, name, desk)
)
EDGE TABLES (
  UBS_POV.clients  AS BelongsTo SOURCE KEY(client_id) REFERENCES Client(client_id)
                   DESTINATION KEY(household_id) REFERENCES Household(household_id) LABEL BELONGS_TO,
  UBS_POV.accounts AS Holds     SOURCE KEY(client_id) REFERENCES Client(client_id)
                   DESTINATION KEY(account_id) REFERENCES Account(account_id) LABEL HOLDS,
  UBS_POV.clients  AS AdvisedBy SOURCE KEY(client_id) REFERENCES Client(client_id)
...
```sql
CREATE OR REPLACE MODEL UBS_POV.attrition_tabularfm
OPTIONS(model_type='TABULARFM', input_label_cols=['attrited']) AS
SELECT * FROM UBS_POV.attrition_training;   -- features: tenure, AuM trend, outflow ratio,
                                            -- dual-banked flag, advisor changes, txn velocity, fees

SELECT client_id, predicted_attrited, predicted_attrited_probs
FROM ML.PREDICT(MODEL UBS_POV.attrition_tabularfm, TABLE UBS_POV.attrition_scoring);
```
...
### 7.5 Conversational Analytics — "Ask UBS" (Gemini Data Analytics agent)
Create a **DataAgent** scoped to `UBS_POV.*` (+ optionally the property graph); stream NL questions; return text + tables + Vega charts + the generated SQL. *(Live: the demo is wired to a real Conversational Analytics data agent — display name `UBS_POV`, id `agent_a548055f-c1cc-4357-bb25-3a16111948cb`, location `global` — via the `:chat` streaming endpoint; the backend maps its `systemMessage` chunks — FINAL_RESPONSE text, `generatedSql`, `result`, Vega `chart`, follow-ups — into the UI block format. Set `CA_AGENT_ID` / `CA_LOCATION` in `backend/.env`.)* **Demo questions:** *"Which booking centre grew NNA fastest last quarter?"* · *"Top 10 UHNW clients by outflow risk."* · *"Compare discretionary vs advisory mandate AuM by region."* · *"Which households hold equities but no discretionary mandate?"* (graph‑backed). **Surface:** `/ask` — chat thread, rendered charts/tables, "show SQL" expander, "pin to Home".
...
FROM AI.SEARCH(TABLE UBS_POV.doc_search, 'chunk_text',
...
df = bpd.read_gbq("UBS_POV.client_features")        # tenure, AuM, asset-mix, txn velocity, ...
from bigframes.ml.cluster import KMeans
m = KMeans(n_clusters=8); m.fit(df); df["segment"] = m.predict(df).to_pandas()
df.to_gbq("UBS_POV.client_segments", if_exists="replace")
```
...
> - **The Data Engineering agent is the REAL Google Cloud Data Engineering Agent** — a genuine A2A agent at `geminidataanalytics.googleapis.com/v1/a2a/projects/{p}/locations/{loc}/agents/dataengineeringagent`, a "BigQuery + Dataform ELT expert." The orchestrator delegates to it over A2A (`message:stream`, with the required `gcpresource` extension pointing at our live Dataform workspace `ubs_pov_pipeline/dev`); its streamed messages appear in the trace marked **LIVE A2A**. See `backend/app/de_agent.py`.
> - **The Conversational Analytics agent (Ask UBS) is also the real product** (data agent `UBS_POV`).
...
## 8. Web Application Design ("UBS Helix")
...
/ask         Ask UBS (Conversational Analytics chat)
...
```
UBS/
  UBS_AGENTIC_POV_PLAN.md      <- this doc
  ...
  infra/
    ...
    setup_ubs_attrition.sql    # TabularFM/boosted-tree churn + scoring + AI plays
    setup_ubs_forecast.sql     # time-series marts (forward-dated for the demo window)
    deploy_cloudrun.sh
    env.example
...
- **Phase 0 — Foundations (Day 1).** Enable APIs (BigQuery, BigQuery Connection, Vertex AI, Storage, Gemini Data Analytics, Cloud Run); create dataset `UBS_POV` + bucket `gs://ubs_pov`; confirm `us-central1.vertex_conn` has Vertex AI User; smoke‑test `AI.GENERATE`.
...
> Thin slice for the workshop (Unify + NBA + Forecast + Ask UBS + Agent Console) is achievable in ~1 week; full polished POV ~2.5–3.5 weeks.

---

## 10. Demo Narrative (the C‑level story)

1. **The two‑bank mess (`/unify`).** "Two banks, two of everything." Run unification → RAW collapses into one Client 360; entity resolution finds the same UHNW client banking with *both* UBS and CS; accuracy gauge ticks up.
...
- **Governance:** all data synthetic; **no real UBS/CS or PII**. Column‑level policy tags as a talking point. Record agent actions for the A2A trace panel.
...
**UBS / customer context**
- [UBS publishes 2025 Annual Report](https://www.ubs.com/global/en/media/display-page-ndp/en-20260309-annual-report.html)
- [Annual Report 2025 — UBS Group (PDF)](https://www.ubs.com/content/dam/assets/cc/investor-relations/annual-report/2025/annual-report-ubs-group-2025.pdf)
- [UBS Group AG — FY2025 investor presentation (SEC 6‑K)](https://www.sec.gov/Archives/edgar/data/0001610520/000161052025000110/investorpresotext2025.htm)
- [UBS Q1 2025 — wealth management drives profit as integration progresses](https://www.investing.com/news/company-news/ubs-q1-2025-presentation-wealth-management-drives-profit-as-integration-progresses-93CH-4011779)
- [UBS — AI for Financial Advisors](https://www.ubs.com/us/en/wealth-management/financial-advisor-experience/articles/ai-for-financial-advisors.html)
- [UBS & Microsoft — co-creating the future of banking with Azure AI](https://www.microsoft.com/en/customers/story/19796-ubs-azure)
- [UBS Group — business structure & CS integration summary](https://exceljump.com/en/ubs-group-guide/)
- [UBS Global Wealth Report 2025](https://www.ubs.com/global/en/media/display-page-ndp/en-20250618-gwr-2025.html)
