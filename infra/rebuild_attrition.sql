-- =====================================================================
-- UBS Helix — rebuild attrition features with a BLENDED flight-risk label
...
-- BigQuery tables, then re-run infra/setup_ubs_attrition.sql to retrain + score.
--
--   bq --project_id=raves-altostrat --location=us-central1 query \
--      --use_legacy_sql=false < infra/rebuild_attrition.sql
--   bq ... < infra/setup_ubs_attrition.sql
-- =====================================================================

CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.attrition_features` AS
WITH flows AS (
  SELECT client_id,
  ...
  FROM `raves-altostrat.UBS_POV.client_flows` GROUP BY client_id
),
acc AS (SELECT client_id, COUNT(*) n_accounts FROM `raves-altostrat.UBS_POV.accounts` GROUP BY client_id),
txn AS (SELECT client_id, COUNT(*) n_txns FROM `raves-altostrat.UBS_POV.transactions` GROUP BY client_id),
feat AS (
...
CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.attrition_training` AS
...
FROM `raves-altostrat.UBS_POV.attrition_features`
WHERE MOD(ABS(FARM_FINGERPRINT(client_id)), 10) < 8;   -- 80% train split

CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.attrition_scoring` AS
