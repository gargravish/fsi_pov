-- =====================================================================
-- UBS Helix — AuM / NNA / revenue forecasting with AI.FORECAST (TimesFM 2.5)
...
--      --use_legacy_sql=false < infra/setup_ubs_forecast.sql
-- =====================================================================

-- Net New Money (the $200bn/yr ambition lever)
CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.forecast_nna` AS
...
CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.forecast_aum` AS
SELECT * FROM AI.FORECAST(
  (SELECT division, region, TIMESTAMP(month) AS month, aum_usd_bn
   FROM `raves-altostrat.UBS_POV.ts_aum_monthly`),
  data_col => 'aum_usd_bn',
  timestamp_col => 'month',
  id_cols => ['division', 'region'],
  model => 'TimesFM 2.5',
...
-- Revenue
CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.forecast_revenue` AS
SELECT * FROM AI.FORECAST(
  (SELECT division, region, TIMESTAMP(month) AS month, revenue_usd_bn
   FROM `raves-altostrat.UBS_POV.ts_revenue_monthly`),
