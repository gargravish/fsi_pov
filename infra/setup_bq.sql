-- =====================================================================
-- UBS Helix — BigQuery AI / embeddings / vector / graph layer
-- Project: raves-altostrat   Dataset: UBS_POV   Region: us-central1
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE MODEL `raves-altostrat.UBS_POV.embedding_model`
REMOTE WITH CONNECTION `raves-altostrat.us-central1.vertex_conn`
OPTIONS (endpoint = 'text-embedding-005');

CREATE OR REPLACE MODEL `raves-altostrat.UBS_POV.gemini_model`
REMOTE WITH CONNECTION `raves-altostrat.us-central1.vertex_conn`
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE VIEW `raves-altostrat.UBS_POV.client_profile` AS
WITH acct AS (
  SELECT client_id, STRING_AGG(DISTINCT account_type, ', ') AS account_types
  FROM `raves-altostrat.UBS_POV.accounts` GROUP BY client_id
),
asset_rank AS (
  SELECT client_id, asset_class,
         ROW_NUMBER() OVER (PARTITION BY client_id ORDER BY SUM(market_value_usd) DESC) rn
  FROM `raves-altostrat.UBS_POV.holdings` GROUP BY client_id, asset_class
),
asset AS (
...
FROM `raves-altostrat.UBS_POV.clients` c
LEFT JOIN acct  USING (client_id)
LEFT JOIN asset USING (client_id);

CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.client_embeddings` AS
SELECT
  client_id,
  ml_generate_embedding_result AS embedding,
  content
FROM ML.GENERATE_EMBEDDING(
  MODEL `raves-altostrat.UBS_POV.embedding_model`,
  (SELECT client_id, content FROM `raves-altostrat.UBS_POV.client_profile`),
  STRUCT(TRUE AS flatten_json_output)
);

-- Product description embeddings (small table -> brute-force VECTOR_SEARCH)
CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.product_embeddings` AS
SELECT
...
FROM ML.GENERATE_EMBEDDING(
  ...
  (SELECT product_id, product_type, name, target_segment_hint,
   ...
   FROM `raves-altostrat.UBS_POV.products`),
  STRUCT(TRUE AS flatten_json_output)
);

-- Document chunk embeddings for Research Brain (AI.SEARCH-style RAG)
CREATE OR REPLACE TABLE `raves-altostrat.UBS_POV.doc_search` AS
SELECT
...
FROM ML.GENERATE_EMBEDDING(
  ...
  (SELECT document_id, doc_type, client_id, title, parsed_text, gcs_uri,
   ...
   FROM `raves-altostrat.UBS_POV.documents`),
  STRUCT(TRUE AS flatten_json_output)
);
...
ON `raves-altostrat.UBS_POV.client_embeddings`(embedding)
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE PROPERTY GRAPH `raves-altostrat.UBS_POV.client_graph`
NODE TABLES (
  `raves-altostrat.UBS_POV.clients` AS Client
    KEY (client_id)
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE VIEW `raves-altostrat.UBS_POV.v_kpis` AS
SELECT
  (SELECT COUNT(*) FROM `raves-altostrat.UBS_POV.clients`) AS clients,
  (SELECT ROUND(SUM(total_aum_usd)/1e9, 1) FROM `raves-altostrat.UBS_POV.clients`) AS aum_usd_bn,
  (SELECT COUNT(*) FROM `raves-altostrat.UBS_POV.accounts`) AS accounts,
  (SELECT ROUND(100*AVG(CAST(dual_banked AS INT64)), 1)
   FROM `raves-altostrat.UBS_POV.clients`) AS dual_banked_pct,
  (SELECT COUNT(*) FROM `raves-altostrat.UBS_POV.advisors`) AS advisors,
  (SELECT ROUND(SUM(net_new_money_usd_bn)*1000, 1)
   FROM `raves-altostrat.UBS_POV.ts_nna_monthly`
